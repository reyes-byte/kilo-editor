...
struct editorSyntax {
    ...
    char *multiline__comment_start;
    char *multiline__comment_end;
    int flags;
};
...
void editorUpdateSyntax(erow *row) {
    ...
    char *mcs = E.syntax->multiline__comment_start;
    char *mce = E.syntax->multiline__comment_end;
    ...
    for (unsigned int j = 0; j < HLDB_ENTRIES; j++) {
        ...
        while (s->filematch[i]) {
            ...
            if ((is_ext && ext && !strcmp(ext, s->filematch[i])) ||
                (!is_ext && strstr(ext, s->filematch[i]))) {
                    E.syntax = s; 
...
void editorInsertChar(int c) {
    if (E.cy == E.numrows) {
        editorInsertRow(E.numrows,"",0); //append a new row (giving a brand new erow)
    }
...
void editorFind() {
    ...
    if (query) {//if esc is pressed
        free(query); //free the buf
    } else {
        E.cx = saved_cx;
        E.cy = saved_cy;
        ...
        if (filerow >= E.numrows) { 
            ...
            for (j = 0; j < len; j++) {
                if (iscntrl(c[j])) {
                    ...
                    abAppend(ab, &sym, 1);
                    abAppend(ab, "\x1b[m", 3);
                    if (current_color != -1) { //check whether there's an existing color
                        char buf[16];
...
void editorDrawStatusBar(struct abuf *ab) {
    ...
    int rlen = snprintf(rstatus, sizeof(rstatus), "%s| %d/%d",
        E.syntax ? E.syntax->filetype :"no ft", E.cy + 1, E.numrows); //add 1 to since E.cy is 0 indexed 
